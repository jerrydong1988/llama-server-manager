...
- 数据保存在独立 `router-usage.db` 中，明细保留 90 天、日汇总保留 365 天。请求检索每页 50 条，支持翻页、精确 UTC 时间、结果、错误类型和关联请求 ID 筛选；这些明细筛选不改变上方汇总与导出。刷新明细后才纳入新记录。后台路由不依赖界面保持打开。统计从新版本开始，不追溯旧数据。
- 点击“查看详情”可以查看失败环节、错误代码、处理建议、实例和接口、用量来源、完整性及各项耗时，并复制诊断摘要。用响应头 `x-lsm-request-id`、`x-request-id` 或 `request-id` 查找对应请求；可用的上游请求 ID 也可精确匹配。客户端自己生成且未传给路由的 Trace ID 无法关联。
- 上下文预检预算与实际消耗分开显示。例如输入 91,675、预留输出 65,536，超过 131,072 的窗口 26,139 Token，应降低客户端输出上限或缩短历史。这类预检拒绝不计生成消耗。未执行或未成功的精确计数保持未知；未设置输出上限时，预检仅按至少 1 Token 预留，不代表引擎最终输出上限。旧记录缺少的诊断字段显示为“—”。
- 按接口查看完整用量覆盖率；写入健康信息覆盖全部调用，不随 Key 和日期筛选变化。待写入数约每 2 秒采样，写入延迟是最近成功批次中从请求完成到落盘的最大延迟。异常结束的采集会话会保留提示，表示可能存在缺口，不能据此推算准确丢失数量。正常退出会刷新记录并标记收尾；首次启用前的异常无法补记。
- 不保存提示词、回答正文或密钥。写入失败／丢失会显示历史提示；突然断电或进程崩溃仍可能丢失尚未落盘的记录。清除所选日期会清除该日期内所有 Key、模型与接口的数据，需要再次确认；运行中随后完成的新请求仍会产生新记录。

Open **Instance Routing → Usage statistics** for per-key requests, reported input/output/cache tokens, accounting coverage, recent calls and daily trends. Filters use completion dates in UTC; displayed call timestamps use your local timezone. Summaries and CSV exports cover the full selected range. Details are retained for 90 days, paginated in groups of 50, and support exact time, outcome, error category and correlated request ID filters independently of summaries. Daily summaries are retained for 365 days. View and copy a request's failure stage, safe category, suggested action and context budget. The `x-lsm-request-id` response header identifies the accounting record; response and upstream IDs can also be searched when available. Preflight budgets are separate from consumption, and a preflight rejection adds no generation tokens. Unmeasured counts and diagnostics missing from older records remain unknown. Per-endpoint coverage and global writer health expose delays and possible gaps after an abnormal recorder exit. Gap counts cannot be reconstructed. Background recording continues without the UI. Cache reads are part of input; partial or unknown usage is never assumed to be zero. Direct instance calls are excluded. No prompts, response content or API secrets are stored.

---

### Key 预算与强制额度 / Key Budgets and Hard Quotas

在 **路由配置 → API Key → 强制 Token 额度** 设置每日／每月上限；默认均为 0（关闭）。软预算只提醒，硬额度不足会在转发前拒绝请求，返回 `token_quota_exceeded`。生成请求必须明确指定正数输出上限，不支持多份生成、批量 Completions 或 `n_predict` 等覆盖参数。无法获得可靠输入计数或额度账不可写时，请求不会转发。

硬额度使用独立 `router-quota.db`，仅记录启用后的调用，不追溯历史；按获得预留时的 UTC 日／月归属，跨日完成仍结算原周期。使用统计中的“已结算”与“待结算／保守占用”分别显示实际结算和预留占用。完整成功响应释放多余预留；取消、断流、错误、未知用量以及崩溃遗留不自动当作零退款。清除使用统计、关闭再开启额度都不重置额度账；未结算占用不带入新周期，可通过提高相应额度继续调用。
