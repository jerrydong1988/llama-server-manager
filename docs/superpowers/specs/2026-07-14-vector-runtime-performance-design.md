...
- 输入吞吐：滚动窗口内完成的输入 token 数 / 窗口秒数。
- 向量项吞吐：滚动窗口内完成的向量任务项数 / 窗口秒数。
...
- 输入吞吐：滚动窗口内完成的 query-document 输入 token 数 / 窗口秒数。
- 文档项吞吐：滚动窗口内完成的重排文档任务项数 / 窗口秒数。
...
第一阶段完成并验证后，另行编写“向量质量评测”设计，讨论数据集来源、Recall@K、MRR、NDCG、基准查询管理和结果可重复性。该设计未经确认不进入实施。
