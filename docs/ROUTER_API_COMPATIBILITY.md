...
- 显式空权限列表拒绝全部操作；旧配置只有缺省 `scopes` 字段时才采用两项默认权限。

请求在鉴权时固定 Token 配额策略；上传或预检期间撤销、禁用 Key 或移除请求所需权限，会在转发前拒绝。相同实例的多个启用路由共享其中最小的非零目标并发上限，默认实例与别名路径一致。每个 Key 最多保留 256 个等待请求，避免独占共享队列。
