...
requireValue(agent.includes('icacls.exe'), 'Windows token ACL hardening must be present')
requireValue(agent.includes('.arg("/reset")'), 'Windows token ACL hardening must remove pre-existing grants')
...
requireValue(agent.includes('MAX_IN_FLIGHT_CONNECTIONS'), 'Agent must cap concurrent connections')
...
requireValue(ui.includes("worker.origin === 'agent'"), 'Cluster UI must identify Agent workers')
requireValue(
...
for (const marker of [
  ...
  'every process on the manager host',
  'audit file is intentionally bounded',
]) {
