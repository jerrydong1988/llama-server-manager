...
const scanEnginesSource = section(coreSliceSource, 'scanEngines:', 'probeEngineCapabilities:')
assert.match(setModelsSource, /modelScanLoading:\s*false/, 'synchronous inventory replacement must end superseded model loading')
...
assert.match(scanModelsSource, /modelScanLoading:\s*true/, 'model scans must own a model-specific loading state')
assert.match(scanEnginesSource, /engineScanLoading:\s*true/, 'engine scans must own an engine-specific loading state')
assert.doesNotMatch(scanModelsSource, /set\(\{\s*isLoading:/, 'model scans must not block unrelated inventory controls')
assert.doesNotMatch(scanEnginesSource, /set\(\{\s*isLoading:/, 'engine scans must not block unrelated inventory controls')
...
assert.match(inventorySource, /MODEL_INVENTORY_SCHEMA_VERSION: i64 = 6/, 'GGUF capability and shard-topology changes must invalidate cached model metadata')
