...
const scannerSource = fs.readFileSync(path.join(root, 'src-tauri', 'src', 'commands', 'scanner.rs'), 'utf8')
...
assert.match(downloadSource, /MAX_HUGGINGFACE_TREE_PAGES/, 'Hugging Face browse must follow bounded pagination')
...
assert.match(proxySource, /MAX_PROXY_HTTP_CLIENTS/, 'proxy HTTP clients must use a bounded cache')
assert.match(proxySource, /exact_context_preflight_required/, 'context preflight must avoid exact tokenization far from the limit')
assert.match(scannerSource, /MODEL_SCAN_LOCK\.lock\(\)\.await/, 'model scans must serialize inventory commits')
...
assert.match(serverSource, /ENGINE_PREVIEW_MATCH_CACHE_TTL/, 'command previews must reuse a short-lived executable verification result')
assert.match(serverSource, /trusted_engine_capabilities\(state\.inner\(\), &config, &engine_exe, false\)/, 'real launches must bypass the preview verification cache')
