...
export function getConfigPageLabels(lang: string) {
  ...
  return {
    ...
    managedModeDesc: zh ? '适用于近期引擎。只传递程序必需参数和偏离程序默认值的调整，其余交给 llama.cpp。' : 'For recent engines. Pass only required arguments and adjustments that differ from app defaults; leave the rest to llama.cpp.',
    ...
    explicitOverridesDesc: zh ? '未修改的参数不传递；修改后才传递；改回程序默认值时自动恢复由引擎处理。' : 'Untouched parameters are omitted, changed values are emitted, and returning to the app default restores engine inheritance.',
    inheritOne: zh ? '恢复引擎默认' : 'Inherit engine default',
    inheritAll: zh ? '恢复全部程序默认' : 'Restore all app defaults',
    noExplicitOverrides: zh ? '当前没有额外调整，启动命令保持精简。' : 'No additional tuning is active; the launch command stays compact.',
    systemArgsHint: zh ? '模型、监听地址、端口及运行观测参数由程序管理；需要强制传递默认值时，可在自定义参数中添加。' : 'Model, endpoint, and observability arguments are app-managed. Use custom arguments only when a default-valued flag must be forced.',
