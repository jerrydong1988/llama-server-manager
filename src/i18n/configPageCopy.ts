...
export function getConfigPageLabels(lang: string) {
  ...
  return {
    ...
    managedModeDesc: zh ? '适用于近期引擎。仅传递程序必需参数和你明确覆盖的参数，其余交给 llama.cpp 默认值。' : 'For recent engines. Pass only application-required arguments and your explicit overrides; inherit the rest from llama.cpp.',
    ...
    explicitOverrides: zh ? '明确覆盖的参数' : 'Explicit Overrides',
    explicitOverridesDesc: zh ? '这些字段会在托管模式中显式进入启动命令。移除覆盖后恢复由引擎默认值处理。' : 'These fields are emitted in managed mode. Remove an override to inherit the engine default again.',
    inheritOne: zh ? '恢复引擎默认' : 'Inherit engine default',
    inheritAll: zh ? '全部恢复引擎默认' : 'Inherit all engine defaults',
    noExplicitOverrides: zh ? '当前没有可选参数覆盖；启动命令仅包含程序必需参数。' : 'No optional overrides. The launch command contains only application-required arguments.',
    systemArgsHint: zh ? '模型、监听地址、端口及程序观测所需参数由系统管理，不计入可选覆盖。' : 'Model, endpoint, and application observability arguments are system-managed and are not counted as optional overrides.',
