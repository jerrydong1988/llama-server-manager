...
export function getProxyLabels(lang: string) {
  ...
  return {
    ...
    keepAliveDesc: zh ? '托盘退出时保持管理器进程在后台运行，避免中断实例路由。当前阶段不是独立 Windows 服务。' : 'Keep the manager process alive from the tray so the route is not interrupted. This phase is not a detached Windows service.',
