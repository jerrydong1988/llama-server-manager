export function getRouterDiagnosticsLabels(lang: string) {
  ...
  return {
    ...
    budgetHint: zh ? '预算不是实际消耗。输出上限未设置时，预检至少预留 1 Token；未执行或未成功的输入计数显示为未知。' : 'Budgets are not consumption. Preflight reserves at least one output token when no limit is set. Skipped or unsuccessful input counts remain unknown.',
