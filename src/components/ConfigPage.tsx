...
const ConfigPage = () => {
  ...
  const selectedTemplateGroups = groupTemplateChanges(selectedTemplateChanges, [
    ...
    { id: 'hardware', title: labels.templateGroupsHardware, keys: ['gpu_layers_auto', 'gpu_layers', 'flash_attn', 'load_mode', 'lazy_mode', 'no_kv_offload', 'kv_unified', 'device', 'split_mode', 'tensor_split', 'main_gpu'] },
      ...
      <div className={manualMode ? 'grid gap-5 2xl:grid-cols-[minmax(0,1fr)_320px]' : 'grid gap-5 xl:grid-cols-[220px,minmax(0,1fr)] 2xl:grid-cols-[220px,minmax(0,1fr)_320px]'}>
        ...
        <div className="space-y-4">
          ...
          <CheckpointPanel config={local} engineExe={currentEngine?.exe} instanceId={configInstanceId || local.id} labels={t.checkpoint}
            ...
            onApplyRequirements={reasons => { set('parallel', 1); set('cache_prompt', true); set('cache_idle_slots', true); if (local.cache_ram !== -1 && local.cache_ram <= 0) set('cache_ram', 8192); set('slots_enabled', true); if (reasons.includes('sliding_window_requires_full_cache')) set('swa_full', true); if (reasons.includes('context_checkpoints_required')) set('ctx_checkpoints', 32) }} />
