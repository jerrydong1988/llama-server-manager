...
const ModelRepo = () => {
  ...
  const handleScan = async () => {
    ...
    if (err) setScanError(err); else setScanError('')
  }
  ...
  const handleAddDirectory = async () => {
    try {
      ...
      if (dir) {
        const all = [...new Set([...modelDirs, dir as string])]
        setModelDirs(all)
  ...
  const handleRemoveDir = async (dir: string) => {
    ...
    setModelDirs(next); scanModels(next)
  }
  ...
  const renderNode = (node: TreeNode, depth: number) => {
      ...
      return (
        <div key={nodeKey}>
          <button onClick={() => { const next = new Set(collapsed); if (next.has(nodeKey)) next.delete(nodeKey); else next.add(nodeKey); setCollapsed(next) }}
            style={{ paddingLeft: `${pl}px` }} className="w-full flex items-center gap-2 py-1.5 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-left">
    ...
    return (
      <div key={nodeKey} style={{ paddingLeft: `${pl}px` }} className="flex items-center gap-2 py-1.5 pr-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
  ...
  return (
    <div className="flex-1 p-6 overflow-y-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t.modelRepo.searchPlaceholder}
              className="pl-10 pr-4 py-2 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900" />
          </div>
          <button onClick={handleScan} disabled={isLoading} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white rounded-lg transition-colors">
            <RefreshCw className={`w-4 h-4 ${isLoading ? 'animate-spin' : ''}`} /> {t.modelRepo.scan}
          </button>
          <button onClick={handleAddDirectory} className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors">
      ...
      {models.length === 0 ? (
        <div className="text-center text-gray-500 py-12">{t.modelRepo.noModels}</div>
      ) : (
        <div>
          ...
          {modelDirs.map(d => (
            <div key={d} className="flex items-center justify-between py-1 px-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700">
              ...
              <button onClick={() => handleRemoveDir(d)} className="p-0.5 text-red-400 hover:text-red-600 rounded" title={t.modelRepo.remove}><Trash2 className="w-3 h-3" /></button>
            </div>
          ))}
