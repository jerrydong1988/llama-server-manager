...
impl syn::parse::Parse for CaptureKind {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        Ok(match keyword.as_str() {
            ...
            k => return Err(syn::Error::new(idents.span(), format!("Unknown keyword `{}`, only `watch`, `weak-allow-none`, `to-owned` and `strong` are allowed",
                k))),
        })
    }
}
    ...
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        if alias.is_none() {
            if name.to_string() == "self" {
                return Err(syn::Error::new(name.span(), "Can't use `self` as variable name. Try storing it in a temporary variable or \
                    rename it using `as`."));
            }
            if name.to_string().contains('.') {
                return Err(syn::Error::new(name.span(), format!("`{}`: Field accesses are not allowed as is, you must rename it!",
                    name)));
            }
        }
        Ok(Capture {
...
impl syn::parse::Parse for Closure {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
            ...
            loop {
                ...
                if capture.kind == CaptureKind::Watch {
                    if let Some(existing) = captures.iter().find(|c| c.kind == CaptureKind::Watch) {
                        { let mut error = syn::Error::new(capture.start, "Only one @watch capture is allowed per closure"); error.combine(syn::Error::new(existing.start, "Previous @watch found here")); return Err(error); };
                    }
                }
        ...
        if closure.asyncness.is_some() {
            return Err(syn::Error::new(closure.span(), "Async closure not allowed"));
        }
        if !captures.is_empty() && closure.capture.is_none() {
            return Err(syn::Error::new(closure.span(), "Closure with captures needs to be \"moved\" so please add `move` before closure"))
        }
        let args = closure
            .inputs
            .iter()
