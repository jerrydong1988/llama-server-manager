...
pub fn impl_shared_boxed(input: &syn::DeriveInput) -> proc_macro2::TokenStream {
    ...
    let refcounted_type = match refcounted_type(input) {
        Some(p) => p,
        _ => {
            return syn::Error::new(proc_macro2::Span::call_site(), "#[derive(glib::SharedBoxed)] requires struct MyStruct(T: RefCounted)").to_compile_error().into()
        }
    };
    ...
    match found {
        Ok(None) => {
            return syn::Error::new(proc_macro2::Span::call_site(), "#[derive(glib::SharedBoxed)] requires #[shared_boxed_type(name = \"SharedBoxedTypeName\")]").to_compile_error().into()
        }
