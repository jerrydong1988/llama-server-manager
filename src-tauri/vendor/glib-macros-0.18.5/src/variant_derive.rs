...
pub fn impl_variant(input: DeriveInput) -> TokenStream {
    match input.data {
        ...
        Data::Enum(data_enum) => {
            let mode = match get_enum_mode(&input.attrs) { Ok(mode) => mode, Err(error) => return error.to_compile_error().into() };
...
fn derive_variant_for_enum(
    ...
    for variant in &data_enum.variants {
        if !matches!(variant.fields, syn::Fields::Unit) && matches!(mode, EnumMode::Enum { .. } | EnumMode::Flags { .. }) {
            return syn::Error::new_spanned(variant, "variant_enum(enum/flags) only supports C-style enums").to_compile_error().into();
        }
    }
...
fn get_enum_mode(attrs: &[syn::Attribute]) -> syn::Result<EnumMode> {
    ...
    let attr = match attr {
        Some(attr) => attr,
        None => return Ok(EnumMode::String),
    };
    ...
    attr.parse_nested_meta(|meta| {
        match meta.path.get_ident().map(|id| id.to_string()).as_deref() {
            ...
            _ => return Err(syn::Error::new_spanned(meta.path, "unknown type in #[variant_enum] attribute")),
        }
        Ok(())
    })?;
    Ok(match mode {
        EnumMode::String if repr_attr.is_some() => {
            ...
            let repr = get_repr(attrs).ok_or_else(|| syn::Error::new_spanned(repr_attr, "Must have #[repr] attribute with one of i8, i16, i32, i64, u8, u16, u32, u64"))?;
            EnumMode::Repr(repr)
        }
