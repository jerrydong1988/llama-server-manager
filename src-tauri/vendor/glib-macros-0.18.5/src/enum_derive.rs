...
pub fn impl_enum(input: &syn::DeriveInput) -> TokenStream {
    ...
    let enum_variants = match input.data {
        ...
        _ => return syn::Error::new(proc_macro2::Span::call_site(), "#[derive(glib::Enum)] only supports enums").to_compile_error().into(),
    };
    ...
    match found {
        Ok(None) => {
            return syn::Error::new(proc_macro2::Span::call_site(), "#[derive(glib::Enum)] requires #[enum_type(name = \"EnumTypeName\")]").to_compile_error().into()
        }
