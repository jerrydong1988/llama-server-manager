...
pub fn impl_error_domain(input: &syn::DeriveInput) -> TokenStream {
    ...
    let enum_variants = match input.data {
        ...
        _ => return syn::Error::new(proc_macro2::Span::call_site(), "#[derive(glib::ErrorDomain)] only supports enums").to_compile_error().into(),
    };
    ...
    match found {
        Ok(None) => {
            return syn::Error::new(proc_macro2::Span::call_site(), "#[derive(glib::ErrorDomain)] requires #[error_domain(name = \"domain-name\")]").to_compile_error().into()
        }
