...
pub fn impl_composite_template(input: &syn::DeriveInput) -> TokenStream {
    ...
    let source = match parse_template_source(input) {
        Ok(v) => v,
        Err(e) => return syn::Error::new(proc_macro2::Span::call_site(), format!("{}: derive(CompositeTemplate) requires #[template(...)] to specify 'file', 'resource', or 'string'",
    ...
    let fields = match input.data {
        ...
        _ => return syn::Error::new(proc_macro2::Span::call_site(), "derive(CompositeTemplate) only supports structs").to_compile_error().into(),
    };
