...
pub(crate) fn is_valid_name_codepoint(code_point: char, first: bool) -> bool {
  if first {
    icu_properties::CodePointSetDataBorrowed::new::<icu_properties::props::IdStart>().contains(code_point) || matches!(code_point, '$' | '_')
  } else {
    icu_properties::CodePointSetDataBorrowed::new::<icu_properties::props::IdContinue>().contains(code_point)
